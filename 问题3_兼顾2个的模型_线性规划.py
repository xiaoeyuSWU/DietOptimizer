...
for o in range(1,8):
    ...
    # 提取相关数据列用于优化
    prices = tem['价格（元/份）'].to_list()
    calories = tem['总能量'].to_list()
    protein = tem['蛋白质'].to_list()
    fat = tem['脂肪'].to_list()
    carbs = tem['碳水化合物'].to_list()
    calcium = tem['钙'].to_list()
    iron = tem['铁'].to_list()
    zinc = tem['锌'].to_list()
    vitamin_a = tem['维生素A'].to_list()
    vitamin_b1 = tem['维生素B1'].to_list()
    vitamin_b2 = tem['维生素B2'].to_list()
    vitamin_c = tem['维生素C'].to_list()
    an = tem[['异亮氨酸', '亮氨酸', '赖氨酸', '含硫氨基酸', '芳香族氨基酸', '苏氨酸', '色氨酸', '缬氨酸']].sum(1).values.tolist()
    ...
    for i in range(len(prices)):
        if x[i].varValue > 0:
            ...
            lss.append(tem.iloc[[i]].index[0])
            nums.append(round(x[i].varValue))
            day.append(o)
            print(f"食品 {i+1}: {round(x[i].varValue)} 份")
    print("总成本：", sum(prices[i] * round(x[i].varValue) for i in range(len(prices))))
    drop_indexx = []
    drop_indexx.append(drop_index[0])
    drop_indexx.append(drop_index[-1])
    drop_indexx.append(drop_index[7])

    tem = tem.drop(tem.iloc[drop_indexx].index,axis=0)
    

result = []
col = [ '价格（元/份）', '蛋白质', '脂肪',
       '碳水化合物', '膳食纤维', '酒精（乙醇）', '钙', '铁', '锌', '维生素A', '维生素B1', '维生素B2',
       '维生素C', '异亮氨酸', '亮氨酸', '赖氨酸', '含硫氨基酸', '芳香族氨基酸', '苏氨酸', '色氨酸', '缬氨酸',
       '蛋白质能量', '脂肪能量', '碳水化合物能量', '膳食纤维能量', '酒精（乙醇）能量', '总能量']
for i in lss:
...
print('-----------------优化结果如下-----------------')
display(result[['餐类', '食物名称','份数','价格（元/份）','星期']])
...
result[['餐类', '食物名称','份数']].to_excel(f'问题3\\{target}{sex}同学优化结果.xlsx')
# 定义数据
...
plt.savefig(f'问题3\\{target}{sex}同学谷物摄入情况.png',dpi=500)
# 显示图形
plt.show()
...
col = ['蛋白质能量',
       '脂肪能量', '碳水化合物能量', '膳食纤维能量', '酒精（乙醇）能量']

tem = (result[col].sum()/result[col].sum().sum())
tem.to_excel(f'问题3\\{target}{sex}同学的日能量贡献.xlsx')
# 定义数据
...
# 创建条形图
plt.style.use('ggplot')  # 选择明亮的风格，如 'ggplot'
...
plt.savefig(f'问题3\\{target}{sex}同学能量摄入情况.png',dpi=500)
# 显示图形
plt.show()
...
tem = (result[col].sum())
tem.to_excel('问题3\\{target}{sex}同学每日膳食非产能主要营养素参考摄入量.xlsx')
...
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.savefig(f'问题3\\{target}{sex}同学非产能摄入情况.png',dpi=500)
# 显示图形
plt.show()
...
for eating in result['餐类'].unique():
    ...
    (result[col].sum()/result['蛋白质'].sum()*10).to_excel(f'问题3\\{target}{sex}氨酸摄入情况.xlsx')
